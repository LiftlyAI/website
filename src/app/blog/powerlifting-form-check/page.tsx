    ...
    },
    {
      ...
      mainEntity: [
        ...
        },
        {
          ...
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'At minimum at the start of each training block, and when introducing a new technical cue. Weekly video review helps lock in a technique change before old patterns re-establish. Avoid filming only at maximal loads — technique breaks down at max effort and the feedback is less useful.',
          },
        },
      ],
    },
    ...
    <>
      ...
      >
        <DefinitionBlock>
          ...
          rules — not to achieve a specific aesthetic.
        </DefinitionBlock>
        ...
        <DataTable>
          ...
          <tbody>
            {[
              ...
              ['Foot width', 'Stance that allows hip depth without grinding at the bottom', 'Too narrow — can\'t reach legal depth'],
              ['Brace', '360° intra-abdominal pressure ("big breath into the belly")', 'Chest breathing only — rigidity lost under load'],
        ...
        <DataTable>
          ...
          <tbody>
            {[
              ['Depth', 'Hip crease below the top of the knee (IPF standard)', '"High squats" — grinding to depth only at max weight'],
              ...
              ['Bar path', 'Vertical over mid-foot throughout the lift', 'Forward drift at the sticking point — increases the moment arm'],
            ].map(([cue, look, fault]) => (
              <tr key={cue}>
                <Td highlight>{cue}</Td>
                <Td>{look}</Td>
        ...
        <DataTable>
          ...
          <tbody>
            {[
              ['Arch', 'Thoracic arch creating a stable base and reducing the bar stroke', 'Flat back — longer stroke, less leg drive contribution'],
              ['Shoulder blades', 'Retracted and depressed — creating a tight, stable shelf', 'Shrugged shoulders — bar path drifts, less stability'],
              ['Leg drive', 'Feet planted, driving force into the setup throughout the press', 'Feet floating or rising — leg drive lost'],
              ['Grip width', 'Competition-legal width (index finger at or inside the 81cm rings)', 'Too wide — shoulder risk; too narrow — more tricep demand'],
            ].map(([cue, look, fault]) => (
              <tr key={cue}>
                <Td highlight>{cue}</Td>
                <Td>{look}</Td>
        ...
        <DataTable>
          ...
          <tbody>
            {[
              ['Touch point', 'Lower chest / solar plexus — varies by anatomy and arch height', 'Touching too high — longer effective stroke'],
              ['Bar path', 'Slight J-curve — descends to chest, pressed back toward the rack at lockout', 'Perfectly vertical — creates more shoulder moment'],
              ['Elbow angle', '45–75° from torso depending on arch and grip width', 'Elbows too flared (>90°) — shoulder impingement risk'],
        ...
        <DataTable>
          ...
          <tbody>
            {[
              ['Bar position', 'Over mid-foot — approximately 1 inch from shins', 'Bar too far forward — creates a longer lever arm off the floor'],
              ['Hip height', 'Hips set with tension, not squatted down or stood over the bar', 'Hips too high at setup — early lower back loading; too low — it becomes a squat'],
              ['Lat engagement', 'Lats engaged ("protect your armpits") before the bar leaves the floor', 'Slack lats — bar swings away from the body during the pull'],
        ...
        <DataTable>
          ...
          <tbody>
            {[
              ['Bar path', 'Vertical — bar drags up the leg the entire pull', 'Bar swinging forward at any point — creates a massive moment arm'],
              ['Back angle', 'Neutral spine maintained throughout — no rounding under load', 'Lumbar rounding (especially off the floor) or thoracic rounding at the top'],
              ['Hip extension', 'Hips drive through at lockout — standing tall', 'Hyperextending backward at lockout — not required and creates injury risk'],
              ['Lockout', 'Knees locked, hips fully extended, standing upright', 'Leaning back excessively or soft knees at lockout — no-lift in competition'],
            ].map(([cue, look, fault]) => (
              <tr key={cue}>
                <Td highlight>{cue}</Td>
                <Td>{look}</Td>
        ...
        <Ul>
          ...
          <Li>Film at normal training weight (70–85% 1RM), not at maximal loads — technique breaks down at near-max efforts and the resulting feedback is less useful for technique work.</Li>
        ...
        <FAQSection>
          ...
          <FAQItem question="How often should I get a form check?">
            ...
            at maximal loads — technique breaks down at max effort and the feedback is less actionable.
          </FAQItem>
        </FAQSection>
      </BlogPost>
    </>
